...
class RAGAgent:
    """RAG Agent for handling product queries using vector search"""
    ...
    def detect_product_intent(self, message: str) -> bool:
        """
        Detect if message is asking for product information.
        
        Args:
            message: User message
            
        Returns:
            True if message is product-related, False otherwise
        """
        product_keywords = [
            "price", "cost", "how much", "show me", "find", "search",
            "what", "which", "tell me about", "do you have", "available",
            "spec", "specification", "feature", "detail", "info", "information"
        ]
    ...
    def process_message(
        ...
        """
        Process user message and return product information.
    ...
    def _build_messages(self, message: str, chat_history: List[ChatMessage]) -> List[Dict[str, Any]]:
...
2. Provide accurate product information including prices, specifications, and availability
3. Help customers find the right products for their needs
4. Be conversational and helpful

Always use the search functions when users ask about products. When presenting product information:
...
If you detect that a user wants to make a purchase (phrases like "I'll take it", "buy", "place order", "confirm"), respond conversationally but let them know that you'll transfer them to our order specialist."""
            }
        ]
    ...
    def should_handoff_to_order_agent(self, message: str, chat_history: List[ChatMessage]) -> bool:
        """
        Determine if conversation should be handed off to order agent.
        ...
        Returns:
            True if should handoff to order agent, False otherwise
        """
        order_intent_phrases = [
            "i'll take it", "i'll buy", "i want to buy", "purchase", "order",
            "add to cart", "checkout", "place order", "confirm", "yes please",
            "i want", "get me", "buy now", "i need"
        ]
        ...
        for phrase in order_intent_phrases:
            if phrase in message_lower:
                ...
                for chat_msg in recent_messages:
                    if any(word in chat_msg.content.lower() for word in ["price", "product", "$", "available"]):
                        return True
