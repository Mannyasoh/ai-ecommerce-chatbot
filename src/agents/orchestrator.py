...
class ConversationOrchestrator:
    """Orchestrates conversation between RAG and Order agents"""
    ...
    def __init__(self):
        ...
        self.conversation_state = "product_inquiry"  # product_inquiry, order_processing

    def determine_agent(self, message: str, chat_history: List[ChatMessage]) -> str:
        """
        ...
        if any(pattern in message_lower for pattern in order_id_patterns):
            # If contains actual order ID format, use order agent
            import re
            if re.search(r"ord-\d{8}-[a-z0-9]{8}|#ord[\d-]+", message_lower):
                return "order_agent"
        
        # Default to RAG agent for product inquiries
        return "rag_agent"

    def _has_product_context(self, chat_history: List[ChatMessage]) -> bool:
        """Check if recent conversation has product context"""
        if not chat_history:
            return False
        
        # Look for product context in last 5 messages
        recent_messages = chat_history[-5:]
        product_indicators = ["price", "$", "product", "available", "stock", "category", "spec"]
            ...
            if selected_agent == "order_agent":
                ...
                if result.get("success") and result.get("function_calls"):
                    for func_call in result["function_calls"]:
                        if (func_call["name"] == "create_order" and 
                            func_call["result"].get("success")):
                            # Add handoff context to response
                ...
                # Check if RAG agent detected order intent for future handoff
                if self.rag_agent.should_handoff_to_order_agent(message, chat_history):
    ...
    def get_conversation_summary(self, chat_history: List[ChatMessage]) -> Dict[str, Any]:
        ...
        # Extract mentioned products and order IDs from content
        import re
        for message in chat_history:
